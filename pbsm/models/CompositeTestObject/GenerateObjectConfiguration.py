...
class CompositeTestObject:
    ...
    def cuboid_inertia(self, dimensions, mass_density, com_pose):
        ...
        inertia_c = (mass / 12) * np.array(
            [[dy**2 + dz**2, 0, 0], [0, dx**2 + dz**2, 0], [0, 0, dx**2 + dy**2]]
        )
    ...
    def cylinder_inertia(self, dimensions, mass_density, com_pose):
        ...
        inertia_c = (mass / 12) * np.array(
            [
                [3 * radius**2 + height**2, 0, 0],
                [0, 3 * radius**2 + height**2, 0],
                [0, 0, 6 * radius**2],
            ]
        )
        inertia_o = self.inertiaWrtS(inertia_c, com_pos, com_ori, mass)
    ...
    # Return the mass and inertia tensor of a half-cylinder with given centre of the full
    # cylinder (located in the centre of the rectangular side)
    ...
    def half_cylinder_inertia(self, dimensions, mass_density, centre):
        ...
        sign = np.sign(centre[1, 3])
        com_pos = centre[0:3, 3] + [0, sign * (4 * radius) / (3 * math.pi), 0]

        mass = math.pi * radius**2 * height * mass_density / 2

        inertia_c = mass * np.array(
            [
                [(1 / 4) * radius**2 + (1 / 3) * height**2, 0, 0],
                [0, (1 / 4) * radius**2 + (1 / 3) * height**2, 0],
                [0, 0, (1 / 2) * radius**2],
            ]
        )
    ...
    def __init__(
        ...
        primitives = [  # Add base plastic cuboids
            cuboid_inertia(
                mm_to_m * np.array([50, 200, 50]),
                PLA_struct_density,
                com_pose(0, 0, 0, mm_to_m * np.array([0, 0, 25])),
            ),
            cuboid_inertia(
                mm_to_m * np.array([200, 50, 50]),
                PLA_struct_density,
                com_pose(0, 0, 0, mm_to_m * np.array([0, 125, 25])),
            ),
            cuboid_inertia(
                mm_to_m * np.array([200, 50, 50]),
                PLA_struct_density,
            ...
            half_cylinder_inertia(
                mm_to_m * np.array([7.5, 50]),
                PLA_struct_density,
                com_pose(0, 0, 0, bolt_pos[0]),
            ),
            half_cylinder_inertia(
                mm_to_m * np.array([7.5, 50]),
                PLA_struct_density,
                com_pose(0, 0, 0, bolt_pos[14]),
            ),
            # Remove plastic (negative density) for weight holes
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[0])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[1])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[2])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[3])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[4])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[5])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[6])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[7])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[8])
            ),
            cylinder_inertia(
                [wh_r, wh_h], -PLA_struct_density, com_pose(0, 0, 0, weight_pos[9])
            ),
            # Remove plastic (negative density) for bolt holes
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[0])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[1])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[2])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[3])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[4])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[5])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[6])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[7])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[8])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[9])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[10])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[11])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[12])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[13])
            ),
            cylinder_inertia(
                [b_r, b_h], -PLA_struct_density, com_pose(0, 0, 0, bolt_pos[14])
            ),
        ]
        ...
        for weight in steel_weights_added:
            i = cylinder_inertia(
                [w_r, w_h], steel_density, com_pose(0, 0, 0, weight_pos[weight])
            )
            primitives.append(i)
        ...
        for weight in plastic_weights_added:
            i = cylinder_inertia(
                [w_r, w_h], ABS_density, com_pose(0, 0, 0, weight_pos[weight])
            )
            primitives.append(i)

        # Compute the global inertial parameters
        total_mass = 0.0
        total_com = np.array([0.0, 0.0, 0.0])
        ...
        if print_params:
            ...
            np.set_printoptions(
                suppress=True, formatter={"float_kind": "{:10.10f}".format}
            )
            print(OUTPUT_URDF)
        ...
        obj_inertia = (
            I_com  # The 3x3 symmetric rotational inertia matrix with respect to the COM
        )
...
def init_argparse():
    parser = argparse.ArgumentParser(
        description="Generate the URDF of a composite object with predictable inertial parameters."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="*",
        help="List of bolt locations.",
    )
    parser.add_argument(
        "-s",
        "--steelweights",
        metavar="POSITION",
        dest="steel_weights_pos",
        type=int,
        nargs="*",
        help="List of weights locations where steel weights are placed.",
    )
    parser.add_argument(
        "-p",
        "--plasticweights",
        metavar="POSITION",
        dest="plastic_weights_pos",
        type=int,
        nargs="*",
        help="List of weights locations where plastic weights are placed.",
    )
    parser.add_argument(
        "-o",
        "--output",
        ...
        dest="output_urdf",
        nargs="?",
        default="GeneratedConfiguration.urdf",
        help="Optional filename of the output URDF to generate.",
    )
    parser.add_argument(
        "--print", help="Prints the resulting inertial parameters.", action="store_true"
    )
    parser.add_argument(
        "--linkxacro",
        help="Generates a XACRO file, that can be included by a parent XACRO, containing only the link.",
        action="store_true",
    )
    parser.add_argument(
